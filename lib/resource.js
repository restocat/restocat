...
var _ = require('lodash')
  ...
  , actions = 'create read update del list'
...
proto.hook = function( stage, path, callback ){
  var resource = this
    , key;

  if(_.isArray( path )){
    _.each(path, function( val ){
      resource.hook( stage, val, callback );
    });
  ...
  if(path) {
    ...
    if (actions.indexOf(path) != -1) {
      path = this._getPathForAction(path);
    }
  }
  ...
  if(_.isArray( callback )){
    _.each(callback, function( val ){
      resource.hooks[key].push( val );
    });

    return;
  }

  resource.hooks[key].push( callback );

  debug( 'hook `' + key + '` added');
};
...
proto._setDefaults = function(){
  ...
  this.beforeAll( hooks.parseValuesByType );
  this.beforeAll( hooks.validateAndSetDefaultMongooseParams );

  this.before( 'create', hooks.checkContentType );
  this.before( 'create', hooks.removeFields );

  this.before( 'update', hooks.checkContentType );
  this.before( 'update', hooks.removeFields );

  this.after( 'list', hooks.addMetaCount );
  this.after( 'list', hooks.addMetaTotal );
...
proto._getPathForAction = function( action ){
  ...
  switch( path ){
    ...
    case 'update':
      ...
      break;
    case 'del':
      path = 'del ' + this.routes.individual;
      break;
    case 'read':
