...
proto.method = function( path, callback ){
  ...
  this[http](uri, function(req, res, next){
    resource._callHooks('before', req, res, path, function( err ){
      ...
      callback.call(resource, req, res, function( err, result ){
        ...
        // call after hooks
        resource._callHooks('after', req, res, result, path, function( err, result ){
          if(err) return next(err);
...
proto._callHooks = function( stage, req, res, result, path, done ){
  var resource = this
    , hooks = []
    , key
    , keyAll;
