...
class Watcher extends EventEmitter {
  ...
  constructor(locator) {
    ...
    if (!this._isRelease) {
      this._events.on('collectionsLoaded', collections => {
  ...
  watchCollections() {
    ...
    return Promise
      .all([this.watchCollectionJsonFiles(), this.watchLogicFiles()])
      .then(watchers => {
        this._events.emit('collectionsWatchReady', watchers);
  ...
  watchCollectionJsonFiles() {
    ...
    this._collectionJsonWatcher = chokidar.watch(this._collectionsFinder.getCollectionsGlob(), CHOKIDAR_OPTIONS)
      ...
      .on('change', filename => {
        ...
        const newCollection = this._collectionsFinder.newCollection(collection.path);
        this.emit('add', newCollection);
      })
  ...
  watchLogicFiles() {
    ...
    this._collectionLogicWatcher = chokidar.watch(dirs, CHOKIDAR_OPTIONS)
      ...
      .on('change', filename => {
        ...
        const relativeLogic = this._collectionsFinder.getRelativeForCollection(collection.path, collection.properties.logic);
