...
class CollectionsFinder extends EventEmitter {
  ...
  watch() {
    ...
    this._fileWatcher = chokidar.watch(Object.keys(this._foundCollectionsByDirs), CHOKIDAR_OPTIONS)
    ...
    chokidar.watch(this._collectionsGlob, CHOKIDAR_OPTIONS)
      .on('error', error => this._events.emit('error', error))
      // add new collection
      .on('add', filename => {
   ...
   *
   * @param collection Collection's descriptor
   * @private
   */
