...
module.exports = {
  ...
  toResult: function (req, res, result, next) {
    var trans = function( val ){
      if(val.constructor.name === 'model')
        return val.toObject({ transform: req.query.transform });

      return val;
    };

    if(_.isArray( result )) {
      result = _.map( result, trans );
    }else{
      result = trans( result );
    }
  ...
  addMetaTotal: function (req, res, result, next) {
    this.model.count(req.query.filters, function(err, total){
