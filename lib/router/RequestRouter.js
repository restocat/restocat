...
class RequestRouter {
   ...
   * @returns {void|Promise} Promise resolved
   */
  requestListener(request, response) {
    ...
    if (!match) {
      ...
      return this.notImplementedHandle(notImplementedContext)
        .catch(reason => reason)
        .then(content => this.format(content, notImplementedContext))
    ...
    return this.callMiddlewares($context)
  ...
  notImplementedHandle($context) {
    ...
    return Promise.reject(
      new errors.NotImplementedError(`Resource or collection '${$context.request.getLocation().path}' not implemented in API`)
    );
  }
