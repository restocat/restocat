...
class RequestRouter {
   ...
   * @returns {{handle: *, state: (*|Array|{index: number, input: string})}} Handle and current state
   */
  ...
  errorHandle(request, response, error) {
    ...
    Promise
      .resolve()
      .then(() => this.format({context, result: error}))
      .then(content => {
      ...
      .catch(error => {
        ...
        response.send(
          `Internal Server Error: During the preparation of the
