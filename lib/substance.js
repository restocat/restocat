...
Substance.prototype._mongooseInit = function(){
  ...
  try {
    ...
    console.error("Mongoose is not found. Please call `npm install mongoose --save");
    process.exit(e.code);
  }
  ...
  _.assign(app, {
    ...
    resource: function( name, schema, options ){
      if(arguments.length > 1){
        ...
        options.defaults = options.defaults || self.options.defaults;

        this._resources[ name ] = new Resource( name, schema, options );

        this.use( this._resources[ name ].getMountPath(), this._resources[ name ] );

        debug( 'added "' + name + '" resources and mount on ' + this._resources[ name ].getMountPath() );
      }
